...
- Eigenvalue & Eigenvector
  ...
  - 고유치분해의 기하학적 의미 :
    행렬 A의 고유벡터(v) -> 선형변환 A에 의해 방향은 보존되고 스케일만 변화되는 방향벡터
    행렬 A의 고유값(λ) -> 고유벡터의 변화되는 스케일 정도
    ==> 행렬 A의 성분에서 방향과 크기를 분리

  - Eigendecomposition
    이러한 고유치분해를 하는 이유는 행렬 A의 대각화 분리가 가능하다는 점에서 이점이 있다.
    고유치분해 후 행렬 A는 다음과 같다.
    Av = λv
    이때, λ는 대각행렬이므로 행렬곱을 했을때 행에 상수배밖에 되지 않는다.
  ...
  - 고유치분해 가능조건 : 일차독립
    'nxn 정방행렬 A가 고유치분해가 가능하려면 행렬 A가 n개의 일차독립인 고유벡터를 가져야한다.'

    '일차독립'이란, 어떤 벡터들의 집합{v1, ..., vn}이 있을 때, 이들 벡터들 중 어느 한 벡터도 다른 벡터들의
    일차결합으로 표현될 수 없으면 이 벡터들을 서로 일차독립이라고 정의한다.
